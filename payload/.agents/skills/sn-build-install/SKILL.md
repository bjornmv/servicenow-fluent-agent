...
---
Build, install, and verify the current now-sdk app against the detected auth alias and project scope. Examples below show `node "$NowSdk" <cmd>` — define `$NowSdk` on the same PowerShell line as each command: `$NowSdk = if (Test-Path 'node_modules\@servicenow\sdk\bin\index.js') { 'node_modules\@servicenow\sdk\bin\index.js' } else { Join-Path $env:APPDATA 'npm\node_modules\@servicenow\sdk\bin\index.js' }; node "$NowSdk" <cmd>`.

## 1. Build
Run `node "$NowSdk" build` — compiles `src/fluent/**/*.now.ts` → `dist/app`. Fast (~10s); install is the slow step, so iterate on build first.

`TSxxxx` / parser errors are BUILD errors (Fluent shape), not install errors. Map each to a [Fluent rule](../../instructions/fluent.instructions.md) and fix at the source (full table in the **sn-fix-build** skill):
- `BarBarToken` / unexpected `||` `&&` `?:` → banned FILE-WIDE (a `const` initializer fails identically): resolve to a literal at authoring time, or move the decision into the external script.
- Inline function body rejected → externalize via `Now.include` with a path relative to the `.now.ts` file, e.g. `Now.include('../../scripts/foo.js')` from `src/fluent/<folder>/foo.now.ts`.
- String `+` concat anywhere in the file (const initializers included) → rewrite as a template-literal `const`.
- `if`/`for`/`while`/`switch`/`var`/`new` in `.now.ts` → move logic to external `.js`/`.ts` + `Now.include`.
...
When unsure of shape, run `node "$NowSdk" explain <recordtype>-api --format raw` first; add `-guide` only for net-new or complex composition. ONE coherent fix → rebuild → repeat in small cycles until clean. Do not touch install until build passes.

## 2. Install
Install only after the build is clean AND the pre-install flow/action scan below is complete. SLOW step (uploads + activates) — let it finish, don't retry on a hang. Install errors are instance-side (auth, scope conflict, activation), not Fluent syntax. Stale auth → re-run **sn-auth**. Do NOT use `--reinstall` (destructive) without confirming first.
...
**Capture exit code AND output — the install can silently fail.** Normal pattern:
```powershell
...
$out = node "$NowSdk" install --auth <alias> 2>&1
$exit = $LASTEXITCODE
$out
$failed = $exit -ne 0 -or (($out | Out-String) -cmatch '(?m)^\[now-sdk\]\s+ERROR:|Could not determine app installation status')
...
```powershell
$NowSdk = if (Test-Path 'node_modules\@servicenow\sdk\bin\index.js') { 'node_modules\@servicenow\sdk\bin\index.js' } else { Join-Path $env:APPDATA 'npm\node_modules\@servicenow\sdk\bin\index.js' }
$out = node "$NowSdk" install --auth <alias> --skip-flow-activation 2>&1
$exit = $LASTEXITCODE
$out
$failed = $exit -ne 0 -or (($out | Out-String) -cmatch '(?m)^\[now-sdk\]\s+ERROR:|Could not determine app installation status')
if ($failed) { throw "now-sdk install failed or reported an error (exit $exit). Check Upgrade History (install --info prints the URL; read rows via sn-rest) before retrying." }
